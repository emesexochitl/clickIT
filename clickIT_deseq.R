...
plotMA(res3[order(res3$padj),], main=paste0("DESeq2 control (Total) and ", mutant, " samples"), ylim = c(1.2*min(res3$log2FoldChange),1.2*max(res3$log2FoldChange)))

png(filename=paste0("maplot_Total_norep_", mutant, "_FDR_01.png"))
...
png(filename=paste0("hist_Total_norep_", mutant, "_", type, ".png"))
hist(res$pval, breaks=100, col="skyblue", border="slateblue", main="")
dev.off()

png(filename=paste0("disp_Total_norep_", mutant, "_", type, ".png"))
plotDispEsts(cds)
dev.off()

write.table(res[order(res$padj), ], file = paste0("deseq_norep_Total_", mutant, "_all.txt"), quote =F)
#write.table((res2[which(res2$padj < 0.05), ]), file = paste0("deseq_Total_", mutant, "_005.txt"), quote=F) 
#write.table((res2[which(res2$padj < 0.05 & res2$log2FoldChange> 1), ]), file = paste0("deseq_Total_", mutant, "_fc1up_005.txt"), quote=F)
#write.table((res2[which(res2$padj < 0.1 & res2$log2FoldChange> 1), ]), file = paste0("deseq_Total_", mutant, "_fc1up_01.txt"), quote=F)
