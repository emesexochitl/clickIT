...
class gene_set():
    ...
    def save_files(self):
        with  open('z_score_feature_total_corr_TPM_only_e.txt', 'a') as f:
            for l in xrange(0, len(self.features)):
                f.write("%s\t%.3f\t%.3f\t%.3f\t%s\t%d\t%.3f\n" % (self.features[l], self.tpms[l], self.z[l],  self.p_values[l], self.sigs[l], self.length, self.mean))
try:
    os.remove("z_score_feature_total_corr_TPM_only_e.txt")
except:
    pass

tpm="feature_total_corr_TPM_vals_0.001_sorted_only_e.txt"
tpmlist=[]
