...
# mostly for plotting, half-life needs new zeros
    ...
    def half_life(self):
        ...
        if  self.max == 0:
            ...
            self.ranging = np.arange(self.min, self.max, 0.000001) # normally it 0.001, but the slope is so radical, can't catxh half-life.
    ...
    def saving(self):
            with  open('model_fit_c2_average_filtering_compound.txt', 'a') as f:
...
try:
    os.remove("model_fit_c2_average_filtering_compound.txt")
except:
    pass
...
idsfile = "average_chron_cluster2_clean.txt"
#idsfile = "test2.txt"
idslist = []
...
for i in ids:
   ...
   print i, gene_id.half_life_y, gene_id.half_life_x
