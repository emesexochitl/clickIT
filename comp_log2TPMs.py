...
output1="comp_iso1_feature_log2TPMplus_vals_fc_with0.txt"
...
for i in xrange(0, len(clicklist)):
    ...
    for j in xrange(0, len(totallist)):
        ...
        elif c1[i] == t1[j] and c2[i] == 0 and t2[j] != 0:
            hit = True
            gene_iso = c1[i]
            c_tpm = 0
            t_tpm = 0
        ...
        elif c1[i] == t1[j] and t2[j] == 0 and c2[i] != 0:
            hit = True
            gene_iso = c1[i]
            c_tpm = 0
            t_tpm = 0
    ...
    if hit == True:
        print gene_iso, c_tpm, t_tpm, log2fc, fc, label, note
